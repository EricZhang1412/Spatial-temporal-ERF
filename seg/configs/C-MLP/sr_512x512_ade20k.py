...
checkpoint_file = "/data/users/sdtv3_zjy/SDT_V3/Segmentation/pretrained/sr_5M_checkpoint-199.pth"
...
model = dict(
    ...
    backbone=dict(
        ...
        type='Spiking_vit_MetaFormer_Spike_SepConv_splash',
        img_size_h=512,
        img_size_w=512,
...
optim_wrapper = dict(
    ...
    type='OptimWrapper',
    optimizer=dict(
        type='AdamW', lr=0.001, betas=(0.9, 0.999), weight_decay=0.005), # lr=0.001
    paramwise_cfg=dict(
        custom_keys={
            'head': dict(lr_mult=2.)}
    ))
