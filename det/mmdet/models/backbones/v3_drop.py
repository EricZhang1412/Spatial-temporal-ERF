...
class MS_Block_Spike_SepConv(nn.Module):
    ...
    def forward(self, x):
        ...
        k_H = int(H*self.k_value)
        k_W = int(W*self.k_value)
        flat_scores = final_score.view(B, -1)  # [B, H*W]
        _, indices = torch.topk(flat_scores, k=k_H*k_W, dim=1)       # [B, kk]
        ...
        slow_x = informative_tokens.reshape(B, C, k_H, k_W)
        slow_x = slow_x + self.attn(slow_x) 
        slow_x = slow_x + self.mlp(slow_x) 

        x = original_x.scatter_(2, indices, slow_x.reshape(B,C,k_H*k_W))  # [B,C,N]
...
class v3_drop(BaseModule):
    def __init__(
        ...
        sr_ratios=[8, 4, 2],
        init_cfg=None,
        k_values=[0.9, 0.9, 0.8, 0.8, 0.8, 0.75, 0.75, 0.75],
    ):
