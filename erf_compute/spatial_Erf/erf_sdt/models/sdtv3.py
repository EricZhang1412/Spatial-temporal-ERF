...
class MultiSpike(nn.Module):
    ...
    def forward(self, x): # B C H W
        return self.spike_function(x, min_value=self.min_value, max_value=self.max_value) / (self.Norm)    # original
...
class MS_ConvBlock_spike_MLP(nn.Module):
    def __init__(
        self,
        dim,
        mlp_ratio=4.0,
        ...
        self.spike1 = MultiSpike()
        self.fc1 = nn.Linear(
            dim, dim * mlp_ratio, bias=False
        )
        self.drop1 = nn.Dropout(drop_probs[0])
        self.spike2 = MultiSpike()
        self.fc2 = nn.Linear(
            dim * mlp_ratio, dim, bias=False
        )
        ...
        # self.spike1 = MultiSpike()
        # self.conv1 = nn.Conv2d(
        #     dim, dim * mlp_ratio, kernel_size=1, bias=False
        # )
        # self.bn1 = nn.BatchNorm2d(dim * mlp_ratio) 
        # self.spike2 = MultiSpike()
        # self.conv2 = nn.Conv2d(
        #     dim * mlp_ratio, dim, kernel_size=1, bias=False
        # )
        # self.bn2 = nn.BatchNorm2d(dim) 
        # ##################################### 
    ...
    def forward(self, x):
        ...
        x = self.spike1(x)
        x = self.drop1(self.fc1(x.reshape(B, H * W, C))).reshape(B, self.mlp_ratio * C, H, W)
        x = self.spike2(x)
        x = self.drop2(self.fc2(x.reshape(B, H * W, self.mlp_ratio * C))).reshape(B, C, H, W)
...
class MS_ConvBlock_spike_splash(nn.Module):
    def __init__(
        self,
        dim,
        mlp_ratio=8.0,
...
class Spiking_vit_MetaFormer_FullAttention(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
...
class Spiking_vit_MetaFormer_B3(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
...
def sdtv3_s_fullattn(**kwargs):
    #5.1M
    model = Spiking_vit_MetaFormer_FullAttention(
        img_size_h=224,
        img_size_w=224,
        embed_dim=[32, 64, 128, 192],
        num_heads=8,
...
def sdtv3_s_attn(**kwargs):
    model = Spiking_vit_MetaFormer_B3(
        img_size_h=224,
        img_size_w=224,
        embed_dim=[32, 64, 128, 192],
        num_heads=8,
...
if __name__ == "__main__":
    ...
    y = model(x)
    torchinfo.summary(model, (1, 3, 224, 224), device='cpu', verbose=1, col_names=["input_size", "output_size", "num_params", "mult_adds"])